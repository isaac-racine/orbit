...
def c_joint_val(
    env: ManagerBasedRLEnv, limval: float, val_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
def c_joint_acc(
    env: ManagerBasedRLEnv, limval: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
def c_action_rate(
    ...
    return (
        torch.amax(torch.abs(env.action_manager.action - env.action_manager.prev_action) / env.step_dt, dim=-1) - limval
    )
