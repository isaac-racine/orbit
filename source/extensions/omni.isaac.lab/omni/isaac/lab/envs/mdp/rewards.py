...
def r_com_linvel_lin(env: RLTaskEnv, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	...
	com_dir = torch.nn.functional.normalize(command[:,:2], dim=-1)
	proj_vel = (linvel[:,:2] * com_dir[:,:2]).sum(dim=-1)
...
def r_anticom_linvel_linmax(env: RLTaskEnv, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	# this reward tries to prevent the robot from rocking back and forth in place to maximize the velocity reward. The weight must be set to <0 to make it a penalty.
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
	linvel: torch.Tensor = asset.data.root_lin_vel_b
	...
	err = torch.clip(-proj_vel, min=0.0)
...
class GaitReward(ManagerTermBase):
	...
	def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRLEnv):
		...
		self.command_name: str = "base_velocity"
