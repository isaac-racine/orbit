...
def r_joint_val_lin(env: ManagerBasedRLEnv, maxerr: float, val_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
...
def undesired_contacts_unit(env: ManagerBasedRLEnv, treshold: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
...
def joint_torques_unit(env: ManagerBasedRLEnv, max_torque: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def joint_acc_unit(env: ManagerBasedRLEnv, max_acc: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def joint_vel_unit(env: ManagerBasedRLEnv, max_vel: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def unflat_orientation_unit(env: ManagerBasedRLEnv, min_dot: float = -1.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def r_flat_orientation_unit(env: ManagerBasedRLEnv, a: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def r_joint_val_exp(env: ManagerBasedRLEnv, maxerr: float, val_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	err = torch.linalg.norm( getattr(asset.data, val_name)[:, asset_cfg.joint_ids], dim=-1 )
	return torch.exp( -(err / (errfac*maxerr))**2 )

def r_joint_jerk_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return r_joint_val_exp(env, maxerr, "joint_jerk", asset_cfg)

def r_joint_acc_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return r_joint_val_exp(env, maxerr, "joint_acc", asset_cfg)

def r_joint_vel_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return r_joint_val_exp(env, maxerr, "joint_vel", asset_cfg)

def r_joint_torque_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def r_flat_orientation_exp(env: ManagerBasedRLEnv, maxerr: float = 2.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	dir_g = torch.nn.functional.normalize(asset.data.projected_gravity_b, dim=-1)
	err = torch.linalg.norm(env.nZ - dir_g, dim=-1) # 2 at max
	return torch.exp( -(err / (errfac*maxerr))**2 )

def r_unflat_orientation_exp(env: ManagerBasedRLEnv, maxerr: float = 2.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	dir_g = torch.nn.functional.normalize(asset.data.projected_gravity_b, dim=-1)
	err = torch.linalg.norm(env.Z - dir_g, dim=-1) # 2 at max
	return torch.exp( -(err / (errfac*maxerr))**2 )

def r_velz_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	err = asset.data.root_lin_vel_b[:, 2]
	return torch.exp( -(err / (errfac*maxerr))**2 )

def r_joint_pose_exp(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
...
def r_dir_xy_exp(
	env: ManagerBasedRLEnv, command_name: str, maxerr: float = 2.0, treshold: float = 0.2, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
...
def rp_joint_val_bilin(env: ManagerBasedRLEnv, minerr: float, maxerr: float, val_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	data = getattr(asset.data, val_name)[:, asset_cfg.joint_ids]
	err = torch.torch.linalg.norm(data , dim=-1)
	return bilin(err, minerr, maxerr)

def rp_joint_acc_bilin(env: ManagerBasedRLEnv, minerr: float, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return rp_joint_val_bilin(env, minerr, maxerr, "joint_acc", asset_cfg)

def rp_joint_vel_bilin(env: ManagerBasedRLEnv, minerr: float, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return rp_joint_val_bilin(env, minerr, maxerr, "joint_vel", asset_cfg)

def rp_joint_torque_bilin(env: ManagerBasedRLEnv, minerr: float, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return rp_joint_val_bilin(env, minerr, maxerr, "applied_torque", asset_cfg)

def rp_flat_orientation_bilin(env: ManagerBasedRLEnv, minerr: float, maxerr: float=2.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	dir_g = torch.nn.functional.normalize(asset.data.projected_gravity_b, dim=-1)
	err = torch.linalg.norm(env.nZ - dir_g, dim=-1) # 2 at max
	return bilin(err, minerr, maxerr)
...
def rp_vel_xy_bilin(
	env: ManagerBasedRLEnv, minerr: float, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
	diff = command[:,:2] - asset.data.root_lin_vel_b[:, :2]
...
def rp_heading_bilin(
	env: ManagerBasedRLEnv, command_name: str, minerr: float, maxerr: float = pi, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
...
def p_joint_val_sparse(env: ManagerBasedRLEnv, treshold: float, val_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	isbad = torch.linalg.norm( getattr(asset.data, val_name)[:, asset_cfg.joint_ids], dim=1 ) > treshold
	return -isbad.float()

def p_joint_acc_sparse(env: ManagerBasedRLEnv, treshold: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return p_joint_val_sparse(env, treshold, "joint_acc", asset_cfg)

def p_joint_vel_sparse(env: ManagerBasedRLEnv, treshold: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	return p_joint_val_sparse(env, treshold, "joint_vel", asset_cfg)

def p_joint_torque_sparse(env: ManagerBasedRLEnv, treshold: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def r_joint_acc_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	
	data = asset.data.joint_acc[:, asset_cfg.joint_ids]
	err = torch.sum(torch.abs(data), dim=-1)
	return lin(err, maxerr)
def r_joint_vel_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	
	data = asset.data.joint_vel[:, asset_cfg.joint_ids]
	err = torch.sum(torch.abs(data), dim=-1)
	return lin(err, maxerr)
def r_joint_torque_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
	
	data = asset.data.applied_torque[:, asset_cfg.joint_ids]
	err = torch.sum(torch.abs(data), dim=-1)
	return lin(err, maxerr)
def r_joint_power_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
...
def r_velz_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	err = torch.abs(asset.data.root_lin_vel_b[:, 2])
	
	return lin(err, maxerr)
def r_acc_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
...
def r_flat_orientation_lin(env: ManagerBasedRLEnv, maxerr: float=2.0, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	
	dir_g = torch.nn.functional.normalize(asset.data.projected_gravity_b, dim=-1)
	err = torch.linalg.norm(env.nZ - dir_g, dim=-1) # 2 at max
	return lin(err, maxerr)
def r_joint_pose_lin(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: Articulation = env.scene[asset_cfg.name]
...
def r_contact_dist_lin(env: ManagerBasedRLEnv, maxerr: float, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
	...
	return lin(err, maxerr)
def r_feetair_velcom_lin(env: ManagerBasedRLEnv, target_t: float, maxerr: float, command_name: str, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
	contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
...
def r_com_linvel_lin(env: ManagerBasedRLEnv, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
	...
	err = torch.linalg.norm(diff, dim=-1)
	return lin(err, maxerr)
def r_com_angvel_lin(env: ManagerBasedRLEnv, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
	...
	err = torch.abs(diff)
	return lin(err, maxerr)
def r_com_heading_lin(env: ManagerBasedRLEnv, command_name: str, maxerr: float = pi, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_command(command_name)
...
def r_com_headingpos_lin(env: ManagerBasedRLEnv, command_name: str, maxerr: float = pi, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_term(command_name).heading_command_b
	
	err = torch.abs(command[:]) # pi at max
	return lin(err, maxerr)
def r_com_eepos_lin(env: ManagerBasedRLEnv, maxerr: float, command_name: str, ee_frame_cfg: SceneEntityCfg = SceneEntityCfg("ee_frame")) -> torch.Tensor:
...
def r_com_pos_lin(env: ManagerBasedRLEnv, maxerr: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	command: torch.Tensor = env.command_manager.get_term(command_name).pos_command_w
...
def r_com_releepos_lin(env: ManagerBasedRLEnv, maxerr: float, command_name: str, 
...
def r_velx_linmax(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
	
	err = torch.abs(asset.data.root_lin_vel_w[:, 0])
	return linmax(err, maxerr)
def r_height_linmax(env: ManagerBasedRLEnv, maxerr: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
	asset: RigidObject = env.scene[asset_cfg.name]
