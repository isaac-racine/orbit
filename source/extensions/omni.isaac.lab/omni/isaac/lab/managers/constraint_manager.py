...
class ConstraintManager(ManagerBase):
    ...
    def __init__(self, cfg: object, env: ManagerBasedRLEnv):
        ...
        for term_name in self._term_names:
            self._c_max[term_name] = 0.0  # torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self._prob_buf = dict()
        for term_name in self._term_names:
            self._prob_buf[term_name] = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
    ...
    def compute(self, dt: float) -> torch.Tensor:
        ...
        for term_name, term_cfg in zip(self._term_names, self._term_cfgs):
            ...
            self._prob_buf[term_name][:] = torch.where(
                torch.isnan(self._prob_buf[term_name]), 0.0, self._prob_buf[term_name]
            )  # happens when 0/0

            # update max termination probability
            self._delta_buf[:] = torch.where(
                self._delta_buf > self._prob_buf[term_name], self._delta_buf, self._prob_buf[term_name]
            )
