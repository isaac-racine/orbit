...
class CommandsCfg:
	...
	ee_pos_cmd = mdp.UniformPoseCommandCfg( #suppose to return quaternion
		...
		ranges=mdp.UniformPoseCommandCfg.Ranges(
			...
			pos_x=(L_MIN, L_MAX), pos_y=(0.0, 0.0), pos_z=(0.0, 0.0), roll=(0.0, 0.0), pitch=(-P_MAX, P_MAX), yaw=(-Y_MAX, Y_MAX)
		),
	)
...
class RewardsCfg:
	...
	#r_loco_alive
	r_loco_alive = RewardTermCfg(func=mdp.is_alive, weight=1.0)
	#####################################################################################################

	################################### MANIPULATION REWARD #############################################

	# r_loco_alive = RewardTermCfg(func=mdp.r_cmd_linvel_lin)
	# 0.2 + (0.5 * base_velocity_cmd)

	r_manip_following = RewardTermCfg(func=mdp.track_pose_orientation, params={"command_name": "ee_pos_cmd"}, weight=0.5)
	...
	# r_joint_power_lin = RewardTermCfg(func=mdp.r_joint__power_lin, params={"maxerr": 3000}, weight=1.0)
	...
	# 		"command_name": "base_velocity_cmd",
	# 	},
	# )
	
	#r__orientation_lin = RewardTermCfg(func=mdp.r__orientation_lin, params={                                   "maxerr": 2.0                 }, weight=1.0)
...
# 	r_cmd_linvel_lin = RewardTermCfg(func=mdp.r_cmd_linvel_lin, params={"command_name": "base_velocity_cmd",               "maxerr": 1.3*MAX_CMD_LINSPEED}, weight=3.0)
# 	r_cmd_angvel_lin = RewardTermCfg(func=mdp.r_cmd_angvel_lin, params={"command_name": "base_velocity_cmd",               "maxerr": MAX_CMD_ANGSPEED    }, weight=3.0)
...
class EnvCfg(EnvCfg):
	...
	# Scene settings
	scene = VelSceneCfg(num_envs=4096, env_spacing=2.5, terrain=terrain)
	# Basic settings
	...
	episode_length_s = _EPISODE_LENGTH
...
@configclass
class EnvCfg_PlayControl(EnvCfg):
	def __post_init__(self):
		super().__post_init__()
		...
		self.commands.base_velocity_cmd.heading_command = True
		self.commands.base_velocity_cmd.vel_world = False
		...
		self.commands.base_velocity_cmd.resampling_time_range = (self.episode_length_s+1, self.episode_length_s+1)
@configclass
class CurriculumEnvCfg_PlayControl(EnvCfg_PlayControl):
	def __post_init__(self):
		super().__post_init__()
