...
# Pre-defined configs
##
from omni.isaac.lab.terrains.config.rough import CUSTOM_TERRAIN_CFG
...
class VelSceneCfg(InteractiveSceneCfg):
    ...
    feet_frame = FrameTransformerCfg(
        ...
        debug_vis=False,
        target_frames=[
            FrameTransformerCfg.FrameCfg(prim_path=f"{{ENV_REGEX_NS}}/Robot/{foot_name}", name=foot_name)
...
class ActionsCfg:
    ...
    legs_pos = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=legs_joints, scale=1.0, use_default_offset=True
    )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObservationGroupCfg):
        ...
        binary_contact = ObservationTermCfg(
            func=mdp.binary_contact, params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=".*_foot")}
        )
...
class FlatRewardsCfg:
    r_com_linvel_lin = RewardTermCfg(
        func=mdp.r_com_linvel_lin,
        params={"command_name": "base_velocity", "maxerr": 1.3 * MAX_COM_LINSPEED},
        weight=3.0,
    )
    r_com_angvel_lin = RewardTermCfg(
        func=mdp.r_com_angvel_lin, params={"command_name": "base_velocity", "maxerr": MAX_COM_ANGSPEED}, weight=3.0
    )
    ...
    r_nbcontact_lin = RewardTermCfg(
        func=mdp.r_nbcontact_lin,
        params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=UNWANTED_CONTACT_BODIES_L)},
        weight=1.0,
    )
